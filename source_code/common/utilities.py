...
def write_file(i, j, beta, theta, time_e, time_m, model_folder, LD2, ld2_list,
    ...
    # write_topic_top_term(list_tops, top_file_name,
    #                      '/home/cuongnb/PycharmProjects/lda_dropout/dataset/twitter/vocab.txt')
    write_topic_top(list_tops, top_file_name)
...
def read_Pw_z(filename):
    # lamda = {}
    fp = open(filename, 'r')
    i = 0
    lamda = 1 * np.random.gamma(100.0, 1.0 / 100.0, (10, 89474))
    while True:
        line = fp.readline()
        # check end of file
        if len(line) < 1:
...
def read_minibatch_list_frequencies_gen_biterm(fp, batch_size):
    word1ids = list()
    word2ids = list()
    stop = 0
    for i in range(batch_size):
        ...
        for j in range(1, doc_length + 1):
            term_count = terms[j].split(':')
            for n in range(len(term_count[1])):
                ids.append(int(term_count[0]))
        for ii in range(0, len(ids) - 1):
            for jj in range(ii + 1, len(ids)):
                word1ids.append(ids[ii])
                word2ids.append(ids[jj])
    return (word1ids, word2ids, stop)
...
def write_topic_top_words_term(beta, tops, top_file_name, file_vocab):
    list_tops = list_top(beta, tops)
    write_topic_top_term(list_tops, top_file_name, file_vocab)


def write_topic_top_term(list_tops, file_name, file_vocab):
    voca = read_voca(file_vocab)
    num_topics = len(list_tops)
    tops = len(list_tops[0])
    f = open(file_name, 'w')
    for k in range(num_topics):
        for j in range(tops - 1):
            f.write('%s ' % (voca[list_tops[k][j]]))
        f.write('%s\n' % (voca[list_tops[k][tops - 1]]))
    f.close()
